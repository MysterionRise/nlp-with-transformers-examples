...
class Check1MinuteSimpleData extends TestKit(ActorSystem("Check1MinData"))
  ...
  actorRef ? Deal(tradeTime.toEpochMilli, "B", 300.0d, 10)
  actorRef ? Deal(tradeTime.toEpochMilli, "C", 200.0d, 10)

  val future = actorRef ? AskFor1MData
  val res = future.value.get.get.asInstanceOf[Data1Minutes]
  res.data should have length 3

  res.data should contain (Some(CandleDeal("A", time , 100.0d, 100.0d, 100.0d, 100.0d, 10)))
  res.data should contain (Some(CandleDeal("B", time , 300.0d, 300.0d, 300.0d, 300.0d, 10)))
  res.data should contain (Some(CandleDeal("C", time , 200.0d, 200.0d, 200.0d, 200.0d, 10)))
