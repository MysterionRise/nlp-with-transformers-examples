...
for model_idx, model_name in tqdm(
    enumerate(model_names), total=len(model_names), desc="Models"
):
    logging.warning(
        f"Processing model {model_idx + 1} of {len(model_names)}: {model_name}"
    )
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModelForSeq2SeqLM.from_pretrained(model_name)

    for file_idx, json_file in tqdm(
        enumerate(json_files), total=len(json_files), desc="Files"
    ):
        logging.warning(
            f"Processing file {file_idx + 1} of {len(json_files)}: {json_file}"
        )
        with open(json_file, "r") as f:
            data = json.load(f)

        texts = [
            item[1] for item in data["raw_answers"]
        ]  # item[1] corresponds to "text"
